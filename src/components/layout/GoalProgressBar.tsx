  ...
  return (
    <div className="bg-card border-b px-3 md:px-4 py-2 md:py-3">
      <div className="max-w-7xl mx-auto">
        ...
        {/* Progress bar */}
        <div className="relative h-4 md:h-5 w-full rounded-full overflow-hidden bg-red-500/80 shadow-inner">
          <div 
          ...
          {percentage >= 20 && (
            <div 
              className="absolute inset-y-0 left-0 flex items-center justify-center text-[10px] md:text-xs font-bold text-white drop-shadow-md"
              style={{ width: `${percentage}%` }}
            >
              {percentage.toFixed(0)}%
            </div>
          )}
          {!isGoalMet && percentage < 80 && (
            <div 
              className="absolute inset-y-0 right-0 flex items-center justify-center text-[10px] md:text-xs font-bold text-white drop-shadow-md"
